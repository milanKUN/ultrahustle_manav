...
const USER_NOTIFICATION_PATH = "/api/v1/me/user-notification";

export const getUserNotification = async () => {
  try {
    const res = await api.get(USER_NOTIFICATION_PATH);
    return unwrap(res);
  } catch (err) {
...
export const putUserNotification = async (payload) => {
  try {
    const res = await api.put(USER_NOTIFICATION_PATH, payload);
    return unwrap(res);
  } catch (err) {
    throw new Error(extractErrorMessage(err));
  }
