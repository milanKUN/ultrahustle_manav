    ...
    <article className="tsl-mp-card" style={{ margin: 0, width: "100%", maxWidth: "100%" }}>
        <div className="tsl-mp-imgWrap">
            <img 
                ...
                alt={p.title} 
                onError={(e) => {
                    e.currentTarget.src = "https://images.unsplash.com/photo-1522542550221-31fd19575a2d?q=80&w=1400&auto=format&fit=crop";
                }}
            />
        </div>
        <div className="tsl-mp-cardBody">
            ...
            <div className="tsl-mp-bottomRow">
                ...
                <button
                    ...
                    type="button"
                    onClick={() => navigate(`/service/${p.listing_username || p.username}`)}
                >
                    {p.cta || "Know More"}
...
const ViewAllServices = ({ theme, setTheme }) => {
    ...
    useEffect(() => {
        const fetchServices = async () => {
            try {
                ...
                if (data.success) {
                    const filtered = data.listings.filter(l => l.listing_type === 'service');
                    ...
                    <div className="relative overflow-y-auto h-[calc(100vh-72px)] w-full">
                        <main className={`alp-main-content ${isDropdownOpen ? "blurred" : ""}`}>
                            <div className="alp-page-container">
                                <h1 className="alp-page-title">Service</h1>
                                {isLoading ? (
                                    <p style={{ color: '#fff', textAlign: 'center', marginTop: '40px' }}>Loading services...</p>
                                ) : (
                                ...
                                {!isLoading && services.length === 0 && (
                                    <p style={{ color: '#fff', textAlign: 'center', marginTop: '40px' }}>No services available.</p>
                                )}
                            </div>
                        </main>
                    </div>
